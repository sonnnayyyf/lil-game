...
export default class Level2Scene extends Phaser.Scene {
    ...
    create() {
        console.log('Level13Scene create');
        ...
        // Add level name
        const levelName = this.add.text(10, topBarHeight / 2, 'Level 13', { fontSize: '24px', fill: '#ffffff' }).setOrigin(0, 0.5).setDepth(1);
        ...
        const wallRadius2 = 70;
        this.circleWall = this.add.image(this.cameras.main.width / 2, this.cameras.main.height / 2-100, 'circleWall').setDisplaySize(wallRadius * 2, wallRadius * 2).setDepth(1);
        this.physics.add.existing(this.circleWall, true);
        this.circleWall.body.setCircle(wallRadius, 0, 0);

        this.circleWall2 = this.add.image(this.cameras.main.width / 2-160, this.cameras.main.height / 2+140, 'circleWall').setDisplaySize(wallRadius2 * 2, wallRadius2 * 2).setDepth(1);
        this.physics.add.existing(this.circleWall2, true);
        this.circleWall2.body.setCircle(wallRadius2, 0, 0);

        // Add gate (brown) and extend it to the bottom of the screen
        const gateHeight = this.cameras.main.height;
        this.gate = this.add.tileSprite(this.cameras.main.width / 2, this.cameras.main.height / 2-300, wallRadius-60, gateHeight, 'gate').setOrigin(0.5);
        this.gate = this.add.tileSprite(this.cameras.main.width / 2-125, this.cameras.main.height / 2-300, wallRadius+125, gateHeight, 'gate').setOrigin(0.5);
        this.physics.add.existing(this.gate, true);

        // Add keys
        this.key1 = this.physics.add.image(this.cameras.main.width / 2-125 , this.cameras.main.height / 2 - 70, 'key').setOrigin(0.5).setScale(0.08).setDepth(1);
        this.key2 = this.physics.add.image(this.cameras.main.width / 2 +125, this.cameras.main.height / 2-70, 'key').setOrigin(0.5).setScale(0.08).setDepth(1);

        // Add door
        this.door = this.physics.add.image(this.cameras.main.width - 550, this.cameras.main.height -400, 'door').setOrigin(0.5).setScale(0.08);
        this.door.body.immovable = true;

        // Add player (Emilee image) under the door
        this.player = this.physics.add.image(this.cameras.main.width - 550, this.cameras.main.height, 'Emilee').setOrigin(0.5).setScale(0.2);
        ...
        this.physics.add.collider(this.player, this.circleWall2);
    ...
    reachDoor(player, door) {
        if (this.keyCount > 0) {
            ...
            this.playerMovementData.push({ x: this.player.x, y: this.player.y });
            GlobalData.savePlayerMovementData('13', this.playerMovementData);
            // Transition to the replay scene
            this.time.delayedCall(100, () => {
                this.scene.start('explanationScene', { playerMovementData: this.playerMovementData });
            });
        } else {
