...
export default class Level2Scene extends Phaser.Scene {
    ...
    create() {
        console.log('Level12Scene create');
        ...
        // Add level name
        const levelName = this.add.text(10, topBarHeight / 2, 'Level 12', { fontSize: '24px', fill: '#ffffff' }).setOrigin(0, 0.5).setDepth(1);
        ...
        // Add door
        this.door = this.physics.add.image(this.cameras.main.width - 225, this.cameras.main.height - 300, 'door').setOrigin(0.5).setScale(0.08);
        this.door.body.immovable = true;

        // Add player (Emilee image) under the door
        this.player = this.physics.add.image(this.cameras.main.width - 225, this.cameras.main.height - 100, 'Emilee').setOrigin(0.5).setScale(0.2);
    ...
    reachDoor(player, door) {
        if (this.keyCount > 0) {
            ...
            this.playerMovementData.push({ x: this.player.x, y: this.player.y });
            GlobalData.savePlayerMovementData('12', this.playerMovementData);
            // Transition to the level 3 scene
            this.time.delayedCall(100, () => {
                this.scene.start('Level13Scene', { playerMovementData: this.playerMovementData });
            });
        } else {
