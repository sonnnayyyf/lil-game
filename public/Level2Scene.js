...
export default class Level2Scene extends Phaser.Scene {
    ...
    create() {
        ...
        // Add level name
        const levelName = this.add.text(10, topBarHeight / 2, 'Level 2', { fontSize: '24px', fill: '#ffffff' }).setOrigin(0, 0.5).setDepth(1);
    ...
    reachDoor(player, door) {
        if (this.keyCount > 0) {
            ...
            this.playerMovementData.push({ x: this.player.x, y: this.player.y });
            GlobalData.savePlayerMovementData('2', this.playerMovementData);
            // Transition to the level 3 scene
            this.time.delayedCall(100, () => {
                this.scene.start('Level3Scene', { playerMovementData: this.playerMovementData });
            });
        } else {
