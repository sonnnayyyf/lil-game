...
export default class Level2Scene extends Phaser.Scene {
    ...
    create() {
        ...
        // Add level name
        const levelName = this.add.text(10, topBarHeight / 2, 'Level 4', { fontSize: '24px', fill: '#ffffff' }).setOrigin(0, 0.5).setDepth(1);
        ...
        // Add keys
        this.key1 = this.physics.add.image(this.cameras.main.width / 2, this.cameras.main.height / 2 + 125, 'key').setOrigin(0.5).setScale(0.08).setDepth(1);
        this.key2 = this.physics.add.image(this.cameras.main.width / 2 +100, this.cameras.main.height / 2+100, 'key').setOrigin(0.5).setScale(0.08).setDepth(1);

        // Add door
        this.door = this.physics.add.image(this.cameras.main.width - 550, this.cameras.main.height - 300, 'door').setOrigin(0.5).setScale(0.08);
    ...
    reachDoor(player, door) {
        if (this.keyCount > 0) {
            ...
            this.playerMovementData.push({ x: this.player.x, y: this.player.y });
            GlobalData.savePlayerMovementData('4', this.playerMovementData);
            // Transition to the level 5 scene scene
            this.time.delayedCall(100, () => {
                this.scene.start('Level5Scene', { playerMovementData: this.playerMovementData });
            });
        } else {
