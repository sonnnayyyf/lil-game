...
export default class Level2Scene extends Phaser.Scene {
    ...
    create() {
        console.log('Level8Scene create');
        ...
        // Add level name
        const levelName = this.add.text(10, topBarHeight / 2, 'Level 8', { fontSize: '24px', fill: '#ffffff' }).setOrigin(0, 0.5).setDepth(1);
        ...
        this.circleWall = this.add.image(this.cameras.main.width / 2, this.cameras.main.height / 2 + 160, 'circleWall').setDisplaySize(wallRadius * 2, wallRadius * 2).setDepth(1);
        this.physics.add.existing(this.circleWall, true);
        this.circleWall.body.setCircle(wallRadius, 0, 0);

        this.circleWall2 = this.add.image(this.cameras.main.width / 2, this.cameras.main.height / 2 + 70, 'circleWall').setDisplaySize(wallRadius * 2, wallRadius * 2).setDepth(1);
        ...
        // Add keys
        this.key1 = this.physics.add.image(this.cameras.main.width / 2 - 120, this.cameras.main.height / 2 +170, 'key').setOrigin(0.5).setScale(0.08).setDepth(1);


        // Add door
        this.door = this.physics.add.image(this.cameras.main.width - 300, this.cameras.main.height - 290, 'door').setOrigin(0.5).setScale(0.08);
        this.door.body.immovable = true;

        // Add player (Emilee image) under the door
        this.player = this.physics.add.image(this.cameras.main.width - 540, this.cameras.main.height-400, 'Emilee').setOrigin(0.5).setScale(0.2);
    ...
    reachDoor(player, door) {
        if (this.keyCount > 0) {
            ...
            this.playerMovementData.push({ x: this.player.x, y: this.player.y });
            GlobalData.savePlayerMovementData('8', this.playerMovementData);
            // Transition to the next level scene
            this.time.delayedCall(100, () => {
                this.scene.start('Level9Scene', { playerMovementData: this.playerMovementData });
            });
        } else {
