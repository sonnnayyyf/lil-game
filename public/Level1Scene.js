...
export default class Level1Scene extends Phaser.Scene {
    ...
    create() {
        ...
        // Add level name
        const levelName = this.add.text(10, topBarHeight / 2, 'Level 1', { fontSize: '24px', fill: '#ffffff' }).setOrigin(0, 0.5).setDepth(1);
    ...
    update(time, delta) {
        ...
        if (this.timerActive) {
            ...
            if (leftOrRight) {
                // Check if player is on the left side of the leftmost key
                if (this.player.x < this.key1.x) {
                    this.timer -= delta * 2;
                }
                // Check if player is on the right side of the rightmost key
                else if (this.player.x > this.key2.x) {
                    this.timer -= delta * 2;
                } else {
                    this.timer -= delta;
                }
    ...
    reachDoor(player, door) {
        if (this.keyCount > 0) {
            ...
            this.playerMovementData.push({ x: this.player.x, y: this.player.y });
            GlobalData.savePlayerMovementData('1', this.playerMovementData);
            // transition to the second scene
            this.time.delayedCall(100, () => {
                this.scene.start('Level2Scene', { playerMovementData: this.playerMovementData });
            });
        } else {
